...
def admin_token():
    ...
    admin_login = client.post("/auth/login", json={"username": "admin", "password": "adminpassword123"})
    seed_token = admin_login.json()["access_token"]
    
    new_admin_name = f"test_admin_{int(time.time() * 1000)}"
    client.post("/auth/create-admin", 
                json={"username": new_admin_name, "password": "password"},
                headers={"Authorization": f"Bearer {seed_token}"})
    
    res = client.post("/auth/login", json={"username": new_admin_name, "password": "password"})
    return res.json()["access_token"]
...
def test_user_cannot_update_status(user_token):
    ...
    res = client.put(f"/appointments/{appt_id}", json=update_data, headers={"Authorization": f"Bearer {user_token}"})
...
def test_admin_can_update_status_only(admin_token, user_token):
    ...
    update_data = {"status": "Confirmed"}
    res = client.put(f"/appointments/{appt_id}", json=update_data, headers={"Authorization": f"Bearer {admin_token}"})
    assert res.status_code == 200
...
def test_cross_user_access_blocked(user_token):
    ...
    client.post("/auth/register", json={"username": user_b_name, "password": "password"})
    ...
    res = client.put(f"/appointments/{appt_b_id}", json={"date": "2029-01-01"}, headers={"Authorization": f"Bearer {user_token}"})
